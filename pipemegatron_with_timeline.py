...
import mpu
import nccl
from utils import set_random_seed, uniform_slice
...
class CUDATimer:
    ...
    def __enter__(self):
        if ENABLE_TIMELIME:
            start = time.time()
            timeline_raw.append((self.event_name, start, "B"))

    def __exit__(self, exc_type, exc_val, exc_tb):
        if ENABLE_TIMELIME:
            torch.cuda.synchronize()
            end = time.time()
            timeline_raw.append((self.event_name, end, "E"))
...
class NCCLTransformer:
    ...
    def forward_step(self, all_inputs, initial_cache_len=0):
        ...
        for batch_id in range(self.n_batch_slices):
            with CUDATimer(f"batch_id={batch_id}"):
                # forward
                slice_batch_size = all_inputs[batch_id, 0].size(1)
                with CUDATimer("create_attention_cache"):
                ...
                for input_id in range(self.n_input_slices):
                    with CUDATimer(f"seq_id={input_id}"):
...
class NCCLTransformerRunner(NCCLTransformer):
    def step(self):
        ...
        with CUDATimer("ALLREDUCE"):
            if self.data_parallel_size > 1:
                # data parallel allreduce
                self.allreduce_params()

        with CUDATimer("UPDATE_WEIGHTS"):
            self.update_weights()

    def run(self, n_steps, warmup_steps=5):
        global ENABLE_TIMELIME
        all_step_times = []
        for i in range(n_steps):
            if i > warmup_steps:
                ENABLE_TIMELIME = True
            start_time = time.time()
            with CUDATimer(f"STEP {i}"):
                self.step()
            torch.cuda.synchronize()
...
def dump_timeline(rank, name):
    dirname = f"timelines/{name}" 
    ...
    import json
    with open(os.path.join(dirname, str(rank)), "w") as f:
...
def main():
    ...
    parser.add_argument('--seq-slices', metavar='EXPR', type=str, default=None)
    ...
    if args.seq_slices is not None:
        seq_slices = ast.literal_eval(args.seq_slices)
    else:
        seq_slices = uniform_slice(config.seq_len, args.n_input_slices)
    batch_slices = uniform_slice(args.batch_size, args.n_batch_slices)
    ...
    dump_timeline(args.rank, "_".join(str(s) for s in seq_slices))
