...
struct MultiheadLMAttentionWithCache : torch::nn::Module {
  ...
  AttentionResultTuple forward(torch::Tensor x, std::unique_ptr<AttentionCache> cache) {
    ...
    std::vector<Tensor> qkv = in_proj->forward(x).contiguous().view({tgt_len, bsz * num_heads_, head_dim_ * 3}).transpose(0, 1).chunk(3, dim=-1);
    Tensor &q = qkv[0];
    Tensor &k = qkv[1];
    Tensor &v = qkv[2];
    ...
    std::unique_ptr<AttentionCache> new_cache = new AttentionCache(k, v);
...
struct TransformerLayer : torch::nn::Module {
  TransformerLayer(int embedding_dim, int num_attention_heads, int ffn_embedding_dim, const torch::Device &device) {
    attn_ln = register_module("attn_ln", torch::nn::LayerNorm(embedding_dim))
    attn = register_module("attn", MultiheadLMAttentionWithCache(embedding_dim, num_attention_heads))
    fc_ln = register_module("fc_ln", torch::nn.LayerNorm(embedding_dim))
  ...
  AttentionResultTuple forward(torch::Tensor x, std::unique_ptr<AttentionCache> attn_cache) {
    ...
    torch::Tensor y = x;
    x = attn_ln->forward(x);
    AttentionResultTuple att = self.attn(x, std::move(attn_cache));
    x = std::get<0>(att);
    x = y + x;
    y = x;
    x = fc_ln->forward(x);
  ...
  MultiheadLMAttentionWithCache attn{nullptr};
